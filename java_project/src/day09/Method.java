...
public class Method {
	...
	public static void main(String[] args) {
		/* 정리
		 * 1. 변수
		 *   - 일반변수(지역변수, 기본형 변수) : 값을 저장하는 기본 메모리 공간
		 *      -정수형 :int, byte short, long (접미사 l(L))
		 *      -실수형:double, float (접미사 f(F))
		 *      -정수형(문자):char(정수형, 야스키코드 문자표에 저장되어 있는 코드로 저장): 1글자
		 ...
		 *   -조건식이 참이면 true, 아니면 false 를 선택
		 *   -간편한 형태 if 역할  
		 * 
		 * 3. 조건문 : 조건식이 참이면 실행문을 실행
		 *   -if, switch 
		 *   -if (조건식): 조건식은 반드시 true/ false 로 구분이 되야 함.
		 *   -switch(값/식) : 값이 나오는 식 , 변수 
		 *      - 하나의 case가 끝나면 break 로 닫아줘야 함
		 *      -break 를 생략하면 하위 모든 case 실행
		 *      
		 *      
		 * 4. 반복문: 반복을 결정하는 식
		 *   -for, while, do~while : 조건식이 true 조건으로 작성    
		 *   -for,while문은 조건식에 따라서 한번도 실행 안될 수 있음.
		 *   -do~while문은 한번은 무조건 실행됨
		 *   
		 *   
		 * 5. break,continue문
		 *    - break; : 반복문을 빠져나가기 위한 구문/ if문을 같이 동반하게 됨
		 *    - continue 문 : pass 하는 역할
		 *       조건이 맞을 때 실해하지 않고, 다음 증감으로 이동
		 *       while 문에서는 증감식이 실행문안에 포함되기 때문에
		 *        조건안에서 증감식을 찾지 못하는 경우 무한루프에 빠질 수 있음.  
		 *        
		 * 6. 배열 
